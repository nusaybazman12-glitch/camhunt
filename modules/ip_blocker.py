"""IP Blocker - blocks a specific IP on the local machine"""
...
class IPBlocker:
    ...
    def block_ip(self, ip):
        ...
        if ip in self.blocked_ips:
            self.logger.warning(f"{ip} already blocked")
            return False, "Already blocked"

        if self.is_termux:
            self.logger.warning("Termux: root required to block IPs")
            return False, "Root required on Termux to block IPs"
        ...
        try:
            if self.os_type == "Linux":
                ...
                if result.returncode == 0:
                    ...
                    self.logger.error(f"Block failed: {result.stderr}")
